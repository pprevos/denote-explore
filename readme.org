...
4. /Visualisations/: Visualise your Denote network (statistics and network graphs).
...
This package is available on MELPA and is easily installed with the ~use-package~ macro. The configuration below sets all available variables. The first two variables define where graph files are stored, sans extension. If the directory does not yet exist, then the package creates it when required. The third variable defines the output format and as such the file extensions. Possible options are =graphviz=, =json= and =gexf=.
...
* Change Log
** Version 1.3
- Complete rewrite of the graph visualisation functionality. Now includes three formats (JavaScript, GraphViz and GEXF) and three graph types (notes that match a regular expression, note neighbourhood and keyword graph). Refer to the manual for details.
...
- Some fine-tuning in GraphViz, especially scaling for large or very small graphs.
...
- Perhaps a plain text time series graph of the creation of new notes could be interesting
