...
The default state is that these functions jump to a Denote file. Adding the universal argument (=C-u=) incudes attachments in the options.
...
I am also looking at a plain text method to visualise the network neighbourhood. The hierarchy package can display information in tree format, which might be useful to visualise the local neighbourhood of a Denote buffer, e.g. two links backwards and forwards.
