...
For example a note with signature =1=a= is the child of a note with signature =1= and a note with signature =1=b= is its sibling. The note with signature =1=a=1= is the child of =1=a= and the grandchild of =1=, and so forth. In a sequence network, links exist independent of any Denote links inside a note, the relationship is only based on the hierarchy of the signatures.
...
Users of the `denote-sequence'  package can generate a sequence graph using ~denote-explore-network~. Select the signature of the root node (note =1= in the diagram) as the starting point. When not selecting any signature, all Denote files with a signature are included in the visualisation.
